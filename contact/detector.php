    ...
    public function onValidateContact($contact, $post) {
        ...
        if ($app->isSite()) {
            ...
            if (JPluginHelper::isEnabled('alikonweb', 'detector')) {
                ...
                $info_detector = $dispatcher->trigger('onDetectText', array(null, $post['contact_email'], $post['contact_name'], $post['contact_message'], ' '));
                if ($info_detector[0]['score'] >= 4) {

                    if (($mail2con == '1') || ($mail2usr == '1')) {
                        $subject = '[Detector4Contact] ' . $post['contact_subject'] . ' from:(' . $post['contact_name'] . ') ';
                        $mbody = JText::sprintf(
                                        'PLG_CONTACT_EMAIL_PENDING_BODY', $post['contact_message'], $post['contact_name'], $info_detector[0]['text'], $post['contact_email']
                        );
                        if ($mail2usr == '1') {
