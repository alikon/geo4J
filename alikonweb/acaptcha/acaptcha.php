...
function plgVerify($p1,$buttonid){
		global $mainframe;
  $plugin =& JPluginHelper::getPlugin('alikonweb', 'acaptcha');$botParams = new JParameter( $plugin->params );
...
function plgAjaxCaptcha_refresh( $tip,$buttonid ) {
	...
	// JPlugin::loadLanguage( 'plg_alikonweb_alikonweb.captchabot',JPATH_ADMINISTRATOR );
	 // Get plugin info
	 $plugin =& JPluginHelper::getPlugin('alikonweb', 'acaptcha');
...
//switch mode begin
	...
	*/
	...
	$link_url	= JURI::base().'plugins/alikonweb/acaptcha/media/';
	//$link_url    =$pippo.'plugins/alikonweb/media/';
	$link_sound	= JURI::base().'plugins/alikonweb/acaptcha/playcode.php?l='.$sound_dir.'&c='.$e.'&x='.$crypt.'&i='.$istanza;
	//$link_sound=$pippo.'plugins/alikonweb/playcode.php?l='.$lang.'&c='.$e.'&x='.$crypt;
	$url_sound	= JRoute::_($link_sound);
	$link_image	= JURI::base().'plugins/alikonweb/acaptcha/showcode.php?'.time().'&c='.$e.'&x='.$crypt.'&i='.$istanza;
...
// refersh	
      ...
      $html.='<img width="24px" height=24px" id="'.
              $btn_refresh.
              '" src="'.
              JURI::root().'plugins/alikonweb/acaptcha/refresh.png" title="'.JText::_( 'REFRESH' ).'" alt="'.JText::_( 'REFRESH' ).'"/>';
