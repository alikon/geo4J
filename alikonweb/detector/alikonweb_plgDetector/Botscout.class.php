...
class Botscout {
    ...
    public function __construct($email, $ip, $api) {
        $this->mail = $email;
        $this->ip = $ip;
        $this->api = $api;
    ...
    function http_botscout() {
        ...
        $XMAIL = urlencode($this->mail);

        // for this example we'll use the MULTI test
        $test_string = "http://botscout.com/test/?multi&mail=$XMAIL&ip=$XIP&key=$APIKEY"; // test IP - reliable
        ////////////////////////
...
//jexit('uno'. $botdata[1].'due'. $botdata[2].'times'. $botdata[3]);
        if (substr($data, 0, 1) == '!') {
            ...
            $response = array('status' => 'ko', 'text' => JText::_('BotScout: Got no info'), 'score' => 0);
        } else {
            if ($botdata[0] == 'Y') {
                $response = array('status' => 'ok', 'text' => JText::sprintf('BotScout: Found in database %s times', $botdata[3]), 'score' => 8);
            } else {
                $response = array('status' => 'ok', 'text' => JText::_('BotScout: Not found in database'), 'score' => 0);
            }
        }
